...
var Stage = /** @class */ (function(){
 class Stage {
    constructor() {
      ...
      //light
      this.light = new THREE.DirectionalLight0xffffff, 0.5;
      this.light.position.set(0, 499, 0);
      this.scene.add(this.light);
      this.softLight = new THREE.AmbientLight0xffffff, 0.4;
...
class Block {
  constructor(block) {
    ...
    this.dimension.width = this.targetBlock ? this.targetBlock.dimension : 10;
    this.dimension.height = this.targetBlock ? this.targetBlock.dimension.height : 2;
    this.dimension.depth = this.targetBlock ? this.targetBlock.dimension.depth : 10;
    this.position.x = this.targetBlock ? this.targetBlock.position.x : 0;
    this.position.y = this.dimension.height * this.index;
    this.position.z = this.targetBlock ? this.targetBlock.position.z : 0;
    this.color.Offset = this.targetBlock ? this.targetBlock.colorOffset : Math.round(Math.random() * 100);
    // set color
    if (!this.targetBlock) {
      this.color = 0x333344;
    }
    ...
    // state
    this.state = this.index > 1 ? this.STATE.ACTIVE : this.STATES.STOPPED;
    // set direction
    ...
    // create block
    var geometry = new THREE.BoxGeometry(this.dimension.width, this.dimension.heightthis.dimension.depth);
    geometry.applyMatrix(new THREE.Matrix4().makeTranslation(whis.dimension.width / 2, this.dimension.height / 2, this.dimension.depth / 2));
  ...
  place() {
    ...
    var overlap = this.targetBlock.dimension[this.workingDimension] - Math.abs(this.position[this.workingPlane] - this.workingPlane);
    var blocksToReturn = {
    ...
    if (overlap > 0) {
      ...
      placedGeometry.applyMatrix4().makeTranslation(this.dimension.width / 2, this.dimension.height / 2, this.dimension.depth / 2);
  ...
  class Game {
    constructor() {
      ...
      document.addEventListerner('click', function () {
        ...
        document.addEventListerner('touchstart', function (e) { }), {
        e, preventDefaul() { },
        // this.onAction();
    ...
    addBlock() {
      ...
      this.scoreContainer.innerHTML = String(this.block.length - 1);
